"""
END-TO-END TRAINING VALIDATION — FULL PROTOCOL
=================================================
3 seeds x 5000 steps x 3 variants = credible result

Variants:
  - softmax (baseline)
  - sp2norm_eager (materialized T×T, fp32 activation)
  - sp2norm_fused (Triton kernel, bf16, no T×T matrix)

Metrics: val loss, perplexity, sink rate, sparsity, wall-clock time
Fused metrics computed via eager diagnostic forward pass (no impact on training).
"""
...
def train_variant(variant, seed, n_embd, n_head, n_layer, max_ctx, batch_size, steps):
    ...
    # Warmup: trigger Triton compilation (not timed)
...
if __name__ == "__main__":
    print("=" * 80, flush=True)
    print("FULL VALIDATION: 3 SEEDS x 5000 STEPS", flush=True)
    print("=" * 80, flush=True)
    ...
    # Store all results: {variant: {seed: (history, time, params)}}
    ...
    # ============================================================
    print(f"\n{'='*80}", flush=True)
    print("FINAL REPORT — AVERAGED OVER 3 SEEDS", flush=True)
    print("=" * 80, flush=True)

    # --- Per-seed final val loss ---
    ...
    for v in variants:
        ...
        sinks, sparses = [], []
        for seed in SEEDS:
            h = all_results[v][seed][0][-1]  # last eval point
        ...
        mean_sparse = sum(sparses) / len(sparses)

        avg_val[v] = (mean_val, std_val)
        avg_time[v] = mean_time
        avg_metrics[v] = {'sink': mean_sink, 'sparse': mean_sparse}
    ...
    # --- Averaged final comparison ---
    print(f"\n  --- Averaged Final Comparison ---", flush=True)
    print(f"  {'Variant':>18s} {'Val':>8s} {'±std':>8s} {'PPL':>8s} {'Sink':>8s} {'Sparse':>8s} {'Time':>8s} {'vs SM':>10s} {'Speedup':>8s}", flush=True)
    print("  " + "-" * 90)

    sm_mean = avg_val['softmax'][0]
    sm_time = avg_time['softmax']

    for v in variants:
        mean_v, std_v = avg_val[v]
        ppl = math.exp(mean_v)
        gap = (mean_v - sm_mean) / sm_mean * 100
        gap_str = "BASELINE" if v == 'softmax' else f"{gap:+.2f}%"
        speedup = sm_time / avg_time[v]
        speed_str = "1.00x" if v == 'softmax' else f"{speedup:.2f}x"
        m = avg_metrics[v]
        print(f"  {v:>18s} {mean_v:8.4f} {std_v:8.4f} {ppl:8.2f} {m['sink']:8.4f} {m['sparse']:8.3f} {avg_time[v]:6.0f}s  {gap_str:>10s} {speed_str:>8s}", flush=True)
    ...
    # --- Fused vs Eager consistency ---
    fused_mean = avg_val['sp2norm_fused'][0]
    eager_mean = avg_val['sp2norm_eager'][0]
    drift = abs(fused_mean - eager_mean) / eager_mean * 100
    print(f"\n  Fused vs Eager drift: {drift:.2f}%", flush=True)

    # --- Statistical significance (simple t-test proxy) ---
    print(f"\n  --- Significance Check ---", flush=True)
    sm_vals = [all_results['softmax'][s][0][-1]['val'] for s in SEEDS]
    for v in ['sp2norm_eager', 'sp2norm_fused']:
        v_vals = [all_results[v][s][0][-1]['val'] for s in SEEDS]
        # All seeds better?
        wins = sum(1 for sv, vv in zip(sm_vals, v_vals) if vv < sv)
        print(f"  {v} beats softmax in {wins}/{len(SEEDS)} seeds", flush=True)
